      ...
      {[1, 2, 3, 4, 5].map((star) => (
        ...
        >
          <Star
            ...
            // 保持星級顏色，但移除 cursor-pointer 等互動類別，避免衝突
...
const NewReviewForm = ({ onReviewSubmit }) => {
  ...
  const [submissionError, setSubmissionError] = useState(''); // 新增狀態來顯示錯誤/成功訊息

  const handleSubmit = (e) => {
    e.preventDefault();
    setSubmissionError(''); // 提交前清除錯誤
    ...
    const newReview = {
        ...
        image: imageFile ? 'https://placehold.co/150x150/06b6d4/ffffff?text=您的上傳照片' : null, // 使用 Placeholder URL
        product: '某個商品',
    };
  ...
  const handleImageChange = (e) => {
    ...
    if (file && file.size < 2 * 1024 * 1024) { // 限制檔案大小 < 2MB
        setImageFile(file);
        setSubmissionError(''); // 清除圖片相關錯誤
    } else {
        setImageFile(null);
...
// 1. 商品總覽頁面 (Dashboard Placeholder)
const HomePage = () => (
  <div className="p-8">
    <h2 className="text-3xl font-bold text-gray-800 mb-6 border-b pb-2">商品總覽 (首頁)</h2>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {[...Array(6)].map((_, i) => (
        <div key={i} className="bg-white p-4 rounded-xl shadow-lg border-l-4 border-sky-400">
          <h3 className="text-xl font-semibold text-gray-800 mb-2">新鮮蔬菜 {i + 1}</h3>
          <p className="text-gray-600 mb-3">當日採收，產地直送，品質保證。</p>
          <button className="bg-sky-500 text-white px-3 py-1 rounded-full text-sm hover:bg-sky-600 transition">
            查看商品
          </button>
        </div>
      ))}
...
const ReviewsPage = () => {
  ...
  const handleReviewSubmit = (newReview) => {
    ...
    // 實際應用中，此處應發起 API 呼叫將數據存入後端/Firestore
